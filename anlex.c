		...
		{//un caracter o una cadena de caracteres
			...
			//	printf("\n%c\n",c);
				...
				else if(c==EOF || c=='\n')
				{
					...
					error(msg);
					fprintf(output,"Se esperaba que finalice el literal ");
					ban = 1;
					while(c!='\n')
                        c=fgetc(archivo);
					break;
			...
			if (ban == 0){
				...
				t.pe = buscar("STRING");
				t.compLex = STRING;
				printf("Numero linea %d caracter %c",numLinea,c);
				printf("ComLex %s-->%d",t.pe->lexema,t.pe->compLex);
				ban = 0;
				break;
			}/*
			t.pe=buscar(id);
...
//	for(i=0;nombreString[i] !=NULL; i++){
	...
	if (encontrado=='s'){
//		printf("%s , %d \n", nombreString[0].descripcion,nombreString[0].codigo);
//		printf("%s ", nombreString[i].descripcion);
		fprintf(output,"%s ",nombreString[i].descripcion);
	}else{
...
/*	int x=0;
	for(x=0;x<10 ; x++){
		printf("%s, %d, \n ", nombreString[x].descripcion,nombreString[x].codigo);
	}
*/	
	if(argc > 1)
	{
